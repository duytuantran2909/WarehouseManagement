...
export function EditProductPage() {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          description: "",
          imageFile: null,
          imagePreview: product.imageUrl ?? product.ImageURL ? `http://localhost:5014${product.imageUrl ?? product.ImageURL}` : "",
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Backend lưu URL tương đối, cần cắt base URL nếu đang xem từ môi trường local
      let imageUrl = formData.imagePreview.startsWith("http") ? formData.imagePreview.replace("http://localhost:5014", "") : formData.imagePreview;

      if (formData.imageFile) {
        // Nếu có ảnh mới thì ưu tiên upload và ghi đè URL cũ
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">Chỉnh sửa thông tin sản phẩm #{id}</p>
        </div>
      </div>
